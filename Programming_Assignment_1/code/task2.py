...
def task2_A():
  ...
  horizon = 30000
  simulations = 50
  regret = task2(UCB,horizon,p1_list,p2,simulations)
  print(regret)
  plt.plot(p2, regret)
  plt.xlabel('p2')
  plt.ylabel('Regret')
  plt.title(f'Regret vs. p2')
  plt.grid(True)
  plt.savefig("task2-A.png")
  plt.clf()

def task2_B():
  delta = 0.1
  p2 = np.arange(0, 0.95 , 0.05)
  p1 = [p + delta for p in p2] 
  horizon = 30000
  num_sims = 50 
  ucb_regret = task2(UCB, horizon, p1, p2, num_sims)
  print(ucb_regret)
  kl_ucb_regret = task2(KL_UCB, horizon, p1, p2, num_sims)
  print(kl_ucb_regret)
  plt.figure(figsize=(12, 6))
  plt.subplot(1, 2, 1)
  plt.plot(p2, ucb_regret)
  plt.xlabel('p2')
  plt.ylabel('Regret')
  plt.title('UCB Regret VS p2')
  plt.grid(True)
  plt.subplot(1, 2, 2)
  plt.plot(p2, kl_ucb_regret)
  plt.xlabel('p2')
  plt.ylabel('Regret')
  plt.title('KL-UCB Regret VS p2')
  plt.grid(True)
  plt.savefig("task2-B.png")
  plt.clf()
